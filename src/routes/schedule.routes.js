...
router.post('/schedule/:userId/slot', requireAdmin, async (req, res) => {
  ...
  const maxHours = await prisma.employeeRestrictions
    ...
    .then((restrictions) => (restrictions ? restrictions.maxConsecutiveHours : 8));

  try {
    if (type === 'work') {
      const consecutiveError = validateConsecutiveHours(startDate, endDate, maxHours);
      if (consecutiveError !== null) {
        return res.status(400).json({ error: consecutiveError });
      }
      const weeklyError = await validateWeeklyHours(userId, weekStart, weekEnd, newSlotInHours);
      if (weeklyError !== null) {
...
router.post('/schedule/:userId/overtime', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur lors de la création de la déclaration d'heure supplémentaire" });
  }
});
...
router.delete('/schedule/overtime/:overtimeId', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur lors de la suppression de la déclaration d'heure supplémentaire" });
  }
});
...
router.put('/schedule/:requestId/change-requests', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Erreur lors de la mise à jour de la demande de changement de planning' });
  }
});
