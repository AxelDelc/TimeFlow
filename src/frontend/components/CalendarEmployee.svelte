<script>
  ...
  let modal = $state(null), selected = $state(null);
  let crDate = $state(''), crStart = $state(''), crEnd = $state(''), crMsg = $state(''), crErr = $state('');
  let otErr = $state(''), otDate = $state(''), otStart = $state(''), otEnd = $state(''), otReason = $state('colleague_abscence');

  function openModal(slot) {
    selected = slot;
    crDate = utcStr(new Date(slot.date)); crStart = timeStr(slot.startTime); crEnd = timeStr(slot.endTime);
    crMsg = ''; crErr = '';
    modal.showModal();
  }
  ...
  async function sendRequest() {
    ...
    const res = await fetch('/employee/schedule/change-request', {
      ...
      body: JSON.stringify({ originalSlotId: selected.id, newDate: crDate, newStartTime: crStart, newEndTime: crEnd, employeeMessage: crMsg }),
    });
    if (res.ok) { modal.close(); alert('Demande envoyée.'); }
    else crErr = "Erreur lors de l'envoi.";
  }

  async function submitOvertime(e) {
    e.preventDefault(); otErr = '';
    const res = await fetch('/employee/overtime', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ date: otDate, startTime: otStart, endTime: otEnd, reason: otReason }),
    });
    if (res.ok) { otDate = otStart = otEnd = ''; otReason = 'colleague_abscence'; alert('Heures supplémentaires déclarées.'); }
  ...
  <div class="sc-calendar">
    ...
    <div class="sc-body">
      ...
      {#each days as day}
        ...
        <div class="sc-day-col" data-date={dateStr} style="height:{(HOUR_END - HOUR_START) * HOUR_HEIGHT}px">
          {#each lines as h}
...
<div class="card card--mb">
  ...
  <div class="card-body">
    ...
    <form onsubmit={submitOvertime} style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr auto;gap:12px;align-items:end">
