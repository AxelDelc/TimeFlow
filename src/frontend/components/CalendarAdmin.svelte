<script>
  ...
  const timeStr = dt => new Date(dt).toLocaleTimeString('fr-FR', { timeZone:'UTC', hour:'2-digit', minute:'2-digit' });
  const toH     = dt => { const d = new Date(dt); return d.getUTCHours() + d.getUTCMinutes() / 60; };
  ...
  const weekLabel   = weekStart.toLocaleDateString('fr-FR', { timeZone:'UTC' }) + ' – ' + new Date(weekStart.getTime() + 6 * DAY_MS).toLocaleDateString('fr-FR', { timeZone:'UTC' });
  ...
  const slotsByDay = untrack(() => {
    ...
    for (const s of slots) { const k = utcStr(new Date(s.date)); if (map[k]) map[k].push(s); }
    return map;
  });
  ...
  let modalAdd = $state(null), modalSlot = $state(null);
  let selected = $state(null);
  let date  = $state(''), start = $state('09:00'), end = $state('10:00'), type = $state('work'), err = $state('');
  // dateDisplay dérivé de date : pas besoin de le maintenir manuellement
  let dateDisplay = $derived(date ? new Date(date + 'T00:00:00Z').toLocaleDateString('fr-FR', { timeZone:'UTC', weekday:'long', day:'numeric', month:'long' }) : '');
  ...
  function openAdd(e, dateStr) {
    ...
    date = dateStr; start = String(h).padStart(2,'0') + ':00'; end = String(Math.min(20,h+1)).padStart(2,'0') + ':00';
    type = 'work'; err = '';
    modalAdd.showModal();
  }

  function openSlot(e, slot) { e.stopPropagation(); selected = slot; modalSlot.showModal(); }
  ...
  async function addSlot() {
    ...
    if (res.ok) { modalAdd.close(); location.reload(); }
  ...
  async function deleteSlot() {
    ...
    if (res.ok) { modalSlot.close(); location.reload(); }
  ...
  <div class="sc-calendar">
    ...
    <div class="sc-body">
      ...
      {#each days as day}
        ...
        <div class="sc-day-col" role="gridcell" tabindex="0" data-date={dateStr}
          ...
          {#each (slotsByDay[dateStr] || []) as slot}
            <div class="sc-slot sc-slot--{slot.type}" role="button" tabindex="0"
...
<div class="card">
  ...
  <div class="card-body">
    <form onsubmit={saveRestrictions} style="display:grid;grid-template-columns:1fr 1fr 2fr auto;gap:12px;align-items:end">
      <div class="form-group" style="margin:0">
        <label class="form-label" for="r-weekly">Heures hebdo cibles</label>
        <input id="r-weekly" type="number" class="form-input" min="1" max="48" step="0.5" bind:value={weekly}>
      </div>
      <div class="form-group" style="margin:0">
        <label class="form-label" for="r-consec">Heures consécutives max</label>
        <input id="r-consec" type="number" class="form-input" min="1" max="12" step="0.5" bind:value={consec}>
      </div>
      <div class="form-group" style="margin:0">
  ...
  {#if selected}
    ...
    <div class="modal-body">
      <p style="color:#475569;font-size:14px;margin-bottom:4px">
        {new Date(selected.date).toLocaleDateString('fr-FR', { timeZone:'UTC', weekday:'long', day:'numeric', month:'long' })}
